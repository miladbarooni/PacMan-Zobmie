...
class Model:
    ...
    def train_agent(self, board: Board, alpha=0.001):
        V_train = 0
        counter = 0
        list_of_past_grids = []
        first_board = None
        ...
        while not board.is_game_over() :
            ...
            self.w_hat_player = self.w_hat_player + alpha * (V_train - V_hat(current_features_player, self.w_hat_player)) * np.array(current_features_player)
...
for i in range(10000):
    if i % 1000== 0:
        ...
        print(model.w_hat_player)
        print(i)
    board = Board()
    model.train_agent(board, 0.01)


print(model.w_hat_player)

write_data('w_hat_player.txt', model.w_hat_player)
